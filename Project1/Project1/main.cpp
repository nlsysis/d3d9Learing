...
static DWORD  currentTime = 0.0f;
static DWORD  lastTime = 0.0f;
static DWORD  startTime = 0.0f;
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
										   ...
										   //メッセージを受け取るまで　プログラムをブロックする
	{
		...
		else
		{
			...
			timeDelta = (currentTime - startTime) /1000.0f;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		if (GetCursorPos(&mousePos))
		{
			...
			g_mouse.position.x = mousePos.x;
			g_mouse.position.y = mousePos.y;
		}
		//g_mouse.position.x = LOWORD(lParam);
		//g_mouse.position.y = HIWORD(lParam);
		break;
	case WM_LBUTTONDOWN:
		g_mouse.isLeftDown = true;
		break;
	case WM_LBUTTONUP:
		g_mouse.isLeftDown = false;
		break;
	case WM_KEYDOWN:
		if (wParam == VK_ESCAPE)
		{
